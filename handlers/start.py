...
async def setup_bot_commands():
    ...
    commands = [
        ...
        types.BotCommand("mode", "🔧 Выбрать режим работы"),
        types.BotCommand("voice", "🔊 Выбрать голос для ответов"),
        types.BotCommand("voices", "📢 Список доступных голосов"),
        types.BotCommand("stats", "📊 Статистика базы знаний"),
        types.BotCommand("reset", "🔄 Очистить историю диалога"),
    ]
...
async def cmd_stats(message: types.Message):
    ...
    try:
        ...
        status = "✅ База знаний готова к использованию!" if total_docs > 0 else "⚠️ База знаний пуста. Добавьте документы в data/documents/"
...
Используйте /mode rag для работы с базой знаний."""
        ...
        await bot.send_message(message.chat.id, stats_text, parse_mode=None)
